...
BENCHMARK_SECS = 60
DONATE_PROB = 0.01
DONATE_ADDRESS = "33s8egcrHZWJs7p22t9CcQwqUfQJTadmbH"
...
def main():
    ...
    argp.add_argument("--list-devices", action="store_true", help="list all devices")
    argp.add_argument(
        "-v",
        "--verbose",
        action="store_true",
    ...
    argp.add_argument(
        ...
        help=(
            "directory for configuration and benchmark files"
            + " (default: ~/.config/nuxhash/)"
        ),
    )
    argp.add_argument("--version", action="store_true", help="show nuxhash version")
...
def run_benchmark(device, algorithm):
    ...
    def report_speeds(sample, secs_remaining):
        ...
        if secs_remaining < 0:
            ...
            print(
                f"  {algorithm.name} {status_line} {speeds} (sampling, {time}) " + "\r",
                end="",
            )
        sys.stdout.flush()
...
class MiningSession(object):
    ...
    def _switch_algos(self):
        ...
        for this_algorithm in self._algorithms:
            this_devices = [
                device
                for device, algorithm in self._assignments.items()
        ...
        if not self._settings["donate"]["optout"] and random() < DONATE_PROB:
            ...
            self._scheduler.enter(
                interval, MiningSession.PROFIT_PRIORITY, self._reset_miners
            )

        self._scheduler.enter(
            interval, MiningSession.PROFIT_PRIORITY, self._switch_algos
        )
