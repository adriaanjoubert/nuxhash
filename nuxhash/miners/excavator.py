...
class ExcavatorServer(object):
    ...
    def __init__(self, executable):
        ...
        self._running_algorithms = {
            algorithm: ESAlgorithm(self, algorithm) for algorithm in ALGORITHMS
        }
    ...
    def start(self):
        ...
        log_thread = threading.Thread(target=miner.log_output, args=(self._process,))
        log_thread.start()
    ...
    def _subscribe(self):
        ...
        self.send_command(
            "subscribe",
            [f"nhmp.{region}.nicehash.com:{NHMP_PORT}", f"{wallet}.{worker}:x"],
        )
    ...
    def _read_devices(self):
        ...
        bus_to_idx = {
            device_data["details"]["bus_id"]: device_data["device_id"]
    ...
    def device_speeds(self, device):
        ...
        device_id = self._device_map[device.pci_bus]
        data = next(
            worker for worker in response["workers"] if worker["device_id"] == device_id
        )
        return {
            algorithm["name"]: algorithm["speed"] for algorithm in data["algorithms"]
        }
...
class ESAlgorithm(ESResource):
    ...
    def _create(self):
        if self._benchmark:
            self._server.send_command("algorithm.add", [f"benchmark-{self._algorithm}"])
        else:
...
class ExcavatorAlgorithm(miner.Algorithm):
    ...
    def __init__(self, parent, excavator_algorithm, **kwargs):
        ...
        miner.Algorithm.__init__(
            self,
            parent,
            name=f"excavator_{excavator_algorithm}",
    ...
    def _transition(self, old, new, detach=lambda x: None, attach=lambda x: None):
        if old != new:
            for x in old - new:
                detach(x)
            for x in new - old:
    ...
    def _start_work(self, device):
        try:
            self.parent.server.start_work(
                self._excavator_algorithm, device, benchmarking=self.benchmarking
            )
    ...
    def current_speeds(self):
        try:
            workers = [
                self.parent.server.device_speeds(device) for device in self._devices
            ]
        except (socket.error, socket.timeout):
            raise miner.MinerNotRunning("could not connect to excavator")
        else:
            ...
            def total_speed(algorithm):
                return sum(
                    [worker[algorithm] for worker in workers if algorithm in worker]
                )

            return [
                total_speed(algorithm)
...
class Excavator(miner.Miner):
    ...
    def __init__(self, config_dir):
        ...
        for algorithm in ALGORITHMS:
            runnable = ExcavatorAlgorithm(
                self, algorithm, warmup_secs=miner.SHORT_WARMUP_SECS
            )
