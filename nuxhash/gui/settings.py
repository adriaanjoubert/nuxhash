...
class SettingsScreen(wx.Panel):
    ...
    def __init__(self, parent, *args, frame=None, **kwargs):
        ...
        self.Bind(wx.EVT_COLLAPSIBLEPANE_CHANGED, self.OnPaneChange, apiCollapsible)
        sizer.Add(
            apiCollapsible, wx.SizerFlags().Border(wx.ALL, main.PADDING_PX).Expand()
        )
        ...
        self._ApiKey = wx.TextCtrl(apiForm, size=(-1, -1), style=wx.TE_PASSWORD)
        self.Bind(wx.EVT_TEXT, self.OnControlChange, self._ApiKey)
        add_valign(apiFormSizer, self._ApiKey, wx.SizerFlags().Expand())

        add_valign(apiFormSizer, wx.StaticText(apiForm, label="API Secret Key Code"))
        self._ApiSecret = wx.TextCtrl(apiForm, size=(-1, -1), style=wx.TE_PASSWORD)
        ...
        sizer.Add(
            advancedForm, wx.SizerFlags().Border(wx.ALL, main.PADDING_PX).Expand()
        )
        advancedSizer = two_col_sizer(3)
        advancedForm.SetSizer(advancedSizer)

        add_valign(
            advancedSizer, wx.StaticText(advancedForm, label="Update interval (secs)")
        )
        self._Interval = wx.SpinCtrl(
            advancedForm, size=(125, -1), min=10, max=300, initial=60
        )
        ...
        self._Threshold = wx.SpinCtrl(
            advancedForm, size=(125, -1), min=1, max=50, initial=10
        )
        self.Bind(wx.EVT_SPINCTRL, self.OnControlChange, self._Threshold)
        add_valign(advancedSizer, self._Threshold)

        add_valign(advancedSizer, wx.StaticText(advancedForm, label="Display units"))
    ...
    def OnSave(self, event):
        ...
        new_settings["nicehash"]["api_organization"] = self._Organization.GetValue()
